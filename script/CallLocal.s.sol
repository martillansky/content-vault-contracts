...
contract CallLocal is Script {
    ...
    ProposalVaultManager proposalVaultManager =
        ProposalVaultManager(vm.envAddress("ANVIL_PROPOSAL_VAULT_MANAGER"));
    SchemaManager schemaManager =
        SchemaManager(vm.envAddress("ANVIL_SCHEMA_MANAGER"));
    MasterCrosschainGranter masterCrosschainGranter =
        MasterCrosschainGranter(
            vm.envAddress("ANVIL_MASTER_CROSSCHAIN_GRANTER")
        );
    ...
    function createProposalVault(bytes32 proposalId) public {
        masterCrosschainGranter.createVaultFromProposal(
            proposalId,
    ...
    function run() external {
        ...
        console2.log(
            "Proposal vault manager owner:",
            proposalVaultManager.owner()
        );
        ...
        console2.log(
            "Has permission read:",
            vault.isPermissionVaultRead(2, wallet0)
        );
        console2.log(
            "Has permission write:",
            vault.isPermissionVaultWrite(2, wallet0)
        );

        console2.log(
            "Balance wallet0 of vault 1:",
            vault.balanceOf(wallet0, 1)
        );
        console2.log(
            "Balance wallet0 of vault 2:",
            vault.balanceOf(wallet0, 2)
        );

        console2.log(
            "Balance wallet1 of vault 2:",
            vault.balanceOf(wallet1, 2)
        );
        ...
        console2.log(
            "Has permission wallet0 to 2:",
            vault.hasGrantedPermission(2, wallet0)
        );
        console2.log(
            "Has permission read:",
            vault.isPermissionVaultRead(2, wallet0)
        ...
        console2.log(
            "Has permission wallet1 to 2:",
            vault.hasGrantedPermission(2, wallet1)
        );
        ...
        proposalVaultManager.upgradePermissionVaultFromProposal(
            proposalId,
            wallet0
        );
        console2.log(
